...
#### 1.3. 类图说明：
&#160; &#160; &#160; &#160;馆藏资源品种的属性有资源名称、国际出版号、价格、简介、馆藏数量、可借数量，图书品种、碟片品种为馆藏资源品种的子类。图书品种的属性有书名、编号、作者、出版社、出版日期、可借天数。碟片品种的属性有碟片名、碟片类型、碟片数、制作公司、可借天数。碟片和图书都属于图书馆资源，拥有馆藏流水号和状态两个属性，每个资源的馆藏流水号是唯一的，状态分为‘借出’和‘未借出’两种。

&#160; &#160; &#160; &#160;图书管理员的属性有姓名、职工号、登录密码，通过密码和职工号登录图书管理系统。借书记录中包含借书时间、应还时间、归还时间和经办人。结束时间是该条借书记录生成的时间，应还时间是根据结束时间和所借资源可借天数自动生成的，归还时间为读者归还资源的时间，若暂未归还则为空。读者借书时由管理员登记，经办人为登记的管理员，若已有预定记录则同时删除该预定记录。

&#160; &#160; &#160; &#160;读者的属性有姓名、性别、身份证号、借书卡号、图书限额、已借图书数、碟片限额、已借碟片数。读者可以借书和预订，借书生成借书记录，预定生成预定记录。预定记录的属性为预定日期、预定时间、预定编号。读者在还书时若超过借书记录中的应还时间则根据罚款细则生成逾期记录，缴纳罚款后才能归还。
